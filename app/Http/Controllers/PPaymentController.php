...
class PPaymentController extends Controller
{
...
$customer->save();
 if(\App\Setting::first()->status==0){
  $charge = $stripe->charges()->create([
    ...
    'amount'   => $amount,
    'description' => \App\User::find(\App\Student::find($request->stud)->data_id)->name.' Book Payment',
]);
}
elseif(\App\Setting::first()->status==1){
 ...
 if(\App\Setting::first()->status==0){
    ...
    "type"=>"service",
    "currency"=>\App\Currency::find(\App\Setting::first()->currency)->code,
    'hosted_checkout' => ['mode' => 'iframe','redirect_uri' => url('')."/ppayment/redirect"]
...
elseif (count(\App\Payment::where('student_id',$request->stud)->where('class',\App\Student::find($request->stud)->class)->where('status','0')->get())>0) {
...
$customer->save();
 ...
 if(count(\App\Payment::where('student_id',$request->stud)->where('class',\App\Student::find($request->stud)->class)->where('status','1')->get())==0){
 	$message=new \App\Payment;
 }  
elseif (count(\App\Payment::where('student_id',$request->stud)->where('class',\App\Student::find($request->stud)->class)->where('status','1')->get())>0) {
$message=new \App\Payment;
}
        $message->student_id=$request->stud;
        $message->class=\App\Student::find($request->stud)->class;
