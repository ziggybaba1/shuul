...
@foreach(\App\Relation::where('parent_id',\App\Parenting::where('user_id',\Auth::user()->id)->first()->id)->get() as $stud)
              <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="d-flex flex-row">
                                    <div class="p-10 p-l-0 b-r">
                                        <h6 class="font-light">@lang('admin.current_class')</h6><b>{{\App\Course::find(\App\Student::find($stud->student_id)->class)->title}}</b></div>
                                    <div class="p-10 b-r">
                <h6 class="font-light">@lang('admin.current_term') @lang('admin.school') @lang('admin.paid_fee') @lang('admin.status')</h6><b>{{\App\Currency::find(\App\Setting::first()->currency)->symbol}}
                     @php
                     $summer=0;
foreach(\App\Fpayment::where('session',\App\Session::latest()->first()->session)->where('term',\App\Session::latest()->first()->terms)->where('student_id',$stud->student_id)->get() as $pay){
      $summer=\App\Feepay::where('pay_id',$pay->id)->where('status','1')->sum('amount');
                ...
                <h6 class="font-light">@lang('admin.current_term') @lang('admin.school') @lang('admin.unpaid_fee') @lang('admin.status')</h6><b>{{\App\Currency::find(\App\Setting::first()->currency)->symbol}}
                     @php
                     $summer=0;
          if(count(\App\Fpayment::where('session',\App\Session::latest()->first()->session)->where('term',\App\Session::latest()->first()->terms)->where('student_id',$stud->student_id)->get())>0){
  foreach(\App\Fpayment::where('session',\App\Session::latest()->first()->session)->where('term',\App\Session::latest()->first()->terms)->where('student_id',$stud->student_id)->get() as $pay){

      $summer=\App\Feepay::where('pay_id',$pay->id)->where('status','!=','1')->sum('amount');
}
        }
        elseif(count(\App\Fpayment::where('session',\App\Session::latest()->first()->session)->where('term',\App\Session::latest()->first()->terms)->where('student_id',$stud->student_id)->get())==0){
         $summer=\App\Fee::where('class',\App\Student::find($stud->student_id)->class)->where('term',\App\Session::latest()->first()->terms)->sum('price');
      }
                ...
                @if($summer!=0)
                <a class="btn btn-primary text-white" href="{{url('')}}/parent/page.fmp?page=11">@lang('admin.make_pay')</a>
                @endif
                                    </div>
                                    <div class="p-10 b-r">
                                        <h6 class="font-light">@lang('admin.library_status')</h6><b>{{count(\App\Borrow::where('collector',$stud->student_id)->where('status','!=','Returned')->where('dom1','student')->get())}}</b>
                                    </div>
                                     <div class="p-10 b-r">
                                        <h6 class="font-light">@lang('admin.book_pay')</h6><b>{{count(\App\Bookassign::where('class',\App\Student::find($stud->student_id)->class)->get())}} </b>
                                        @if(count(\App\Payitem::where('student_id',$stud->student_id)->where('class',\App\Student::find($stud->student_id)->class)->get())!=count(\App\Bookassign::where('class',\App\Student::find($stud->student_id)->class)->get()))
                                        <a class="btn btn-success text-white" href="{{url('')}}/parent/page.fmp?page=12">@lang('admin.buy_book')</a>
                                        @endif
                                       
                                    </div>
                                      <div class="p-10 b-r">
                                        <h6 class="font-light">@lang('admin.attend_status')</h6><b>@lang('admin.present'): {{count(\App\Attendance::where('student_id',$stud->student_id)->where('status','1')->where('session',\App\Session::latest()->first()->session)->where('term',\App\Session::latest()->first()->terms)->get())}},&nbsp; &nbsp; &nbsp;@lang('admin.absent'): {{count(\App\Attendance::where('student_id',$stud->student_id)->where('status','0')->where('session',\App\Session::latest()->first()->session)->where('term',\App\Session::latest()->first()->terms)->get())}} </b>
            ...
            @endif
               ...
               <div class="col-lg-12 col-md-12">
                        <div class="card bg-inverse">
                            ...
                            @forelse(\App\Noticeboard::where('status','0')->orWhere('status','2')->orWhere('status','6')->get() as $notice)
                            @if($loop->iteration==1)
