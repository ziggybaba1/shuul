<div class="row">
     ...
     <ul class="nav nav-tabs profile-tab" role="tablist">
                            ...
                            <div class="card-body">
      <h4 class="card-title">{{\App\Student::find($id)->gname}} {{\App\Student::find($id)->fname}} School Fee Payment ({{\App\Course::find(\App\Student::find($id)->class)->title}})</h4>
...
@if(count(\App\Fpayment::where('student_id',$id)->where('session',\App\Session::latest()->first()->session)->where('term',\App\Session::latest()->first()->terms)->get())==0)
      <form action="{{url('')}}/pinitiate/payment/plan/{{$id}}" id="myContainer" method="post" class="form-horizontal m-t-40 formpay" class="form">
                                 {{ csrf_field() }}
...
@foreach(\App\Fee::where('class',\App\Student::find($id)->class)->where('term',\App\Session::first()->terms)->get() as $ent)
       ...
       <tfoot>
            <tr>
              ...
              <td><input name="total" style="display: none;" required  readonly type="text" class="form-control col-md-7 col-xs-12 totalld"><input type="number" style="display: none;" value="{{count(\App\Fpayment::where('student_id',$id)->where('session',\App\Session::first()->session)->where('term',\App\Session::first()->terms)->get())+1}}" name="index"></td>
              <td></td>
            </tr>
          </tfoot>
  </table>
...
<button onclick="resetpat('{{\App\Fpayment::where('student_id',$id)->where('session',\App\Session::first()->session)->where('term',\App\Session::first()->terms)->first()->id}}')" class="btn btn-dark">Reset Payment Plan</button>
                                </div>
                                <br>
                                <div class="table-responsive m-t-40">
