                    ...
                    <ul class="navbar-nav my-lg-0">
                        ...
                        <li class="nav-item dropdown">
                           <a class="nav-link dropdown-toggle text-muted waves-effect waves-dark" href="" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                 @if(\App\Parenting::where('user_id',\Auth::user()->id)->first()->image=='')
                                  <img src="{{url('')}}/assets/images/user.png" alt="user" class="profile-pic" />
                                   @else
        <img src="{{url('')}}/uploads/avatars/{{\App\Parenting::where('user_id',\Auth::user()->id)->first()->image}}" alt="user" class="profile-pic" />
        @endif
                ...
                @else
                 <img src="{{url('')}}/uploads/avatars/{{\App\Parenting::where('user_id',\Auth::user()->id)->first()->image}}" alt="user">
                @endif
                                            </div>
                                            <div class="u-text">
                                                <h4>{{\Auth::user()->name}}</h4>
        <p class="text-muted">{{\Auth::user()->email}}</p><a href="{{url('')}}/parent/page.fmp?page=18" class="btn btn-rounded btn-danger btn-sm">@lang('admin.view') @lang('admin.profile')</a></div>
                                        </div>
                                    </li>
            ...
            <div class="scroll-sidebar">
                ...
                <nav class="sidebar-nav">
                    <ul id="sidebarnav">
                        ...
                        <li>
    <a href="{{url('')}}/parent/page.fmp?page=1" ><i class="mdi mdi-gauge"></i><span class="">@lang('parent.dashboard')</span></a>
                        </li>
                          <li>
        <a class="has-arrow" href="#" aria-expanded="false"><i class="mdi mdi-bullseye"></i>@lang('parent.academic')</a>
                                    <ul aria-expanded="false" class="collapse">
                                         @if(\App\Support::first()->res=='1')
<li><a href="{{url('')}}/parent/page.fmp?page=2" >@lang('parent.result')</a></li>
@endif
<li><a href="{{url('')}}/parent/page.fmp?page=3" >@lang('parent.routine')</a></li>
</ul>
</li>
            <li>
        <a class="has-arrow" href="#" aria-expanded="false"><i class="mdi mdi-chart-bubble"></i><span class="hide-menu">@lang('admin.record')</span></a>
                ...
                <li><a href="{{url('')}}/parent/page.fmp?page=6" >@lang('parent.attendance-over')</a></li>  
                @endif
                 @if(\App\Support::first()->pick=='1')            
                <li><a href="{{url('')}}/parent/page.fmp?page=7" >@lang('parent.pickup-over')</a></li> 
                @endif
                 @if(\App\Support::first()->awa=='1')
                <li><a href="{{url('')}}/parent/page.fmp?page=8" >@lang('parent.student-award')</a></li> 
                @endif
                            </ul>
                        </li> 
        <li>
    <a class="has-arrow " href="#" aria-expanded="false"><i class="mdi mdi-widgets"></i><span class="hide-menu">@lang('admin.account')</span></a>
                            <ul aria-expanded="false" class="collapse">
                                <li><a href="{{url('')}}/parent/page.fmp?page=11">@lang('admin.school') @lang('admin.fee') @lang('admin.payment')</a></li>
                                <li><a href="{{url('')}}/parent/page.fmp?page=12">@lang('admin.book') @lang('admin.payment')</a></li>
                            </ul>
        </li>
         @if(\App\Support::first()->event=='1')
                                  <li><a href="{{url('')}}/parent/page.fmp?page=16"><i class="fa fa-calendar"></i> @lang('parent.event')</a></li>
                                  @endif
                                   @if(\App\Support::first()->gall=='1')
                                   <li><a href="{{url('')}}/parent/page.fmp?page=20"><i class="fa fa-file"></i> @lang('admin.gallery')</a></li>
                                @endif
                                 @if(\App\Support::first()->forum=='1')
                                   <li><a href="{{url('')}}/parent/page.fmp?page=17"><i class="mdi mdi-message"></i> @lang('admin.forum')</a></li>
                                   @endif

                                    <li><a href="{{url('')}}/parent/page.fmp?page=19"><i class="fa fa-inbox"></i> @lang('parent.inbox') <span class="badge badge-success pull-right">{{count(\App\Messaging::where('receive_id',\Auth::user()->id)->where('status','1')->get())}}</span></a></li>
                    </ul>
                </nav>
        ...
        <div class="modal-dialog modal-lg">
            <div  class="come modal-content" style="height:auto; overflow:auto;">
                <div class="modal-header">
                    ...
                    <button id="first" type="button" class="hider btn btn-danger btn-sm" data-dismiss="modal">Close</button>
                </div>
                ...
                <div class="modal-footer">
                    <button type="button" class="hider btn btn-default" data-dismiss="modal">Close</button>
                </div>
            </div>
        </div>
    </div>
...
<script type="text/javascript" src="https://www.wepay.com/js/iframe.wepay.js">
 ...
 $(document).ready(function() {
  ...
  var id = $(this).find("option:selected").attr("id");
   ...
   $.ajax({
    url: '{{url('')}}/get_student/data/'+searchvalue+'/'+id,
    ...
    if(countryID){
        $.ajax({
           ...
           success:function(res){               
            if(res){
                $("#studentdetail").empty();
                $("#studentdetail").append('<option value="all">All Student</option>');
