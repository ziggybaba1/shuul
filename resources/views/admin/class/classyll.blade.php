...
<div class="row">
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <h6 class="card-subtitle"></h6>
                                 ...
                                 <button data-toggle="modal" data-target=".bs-example-modal-lg" class="btn btn-primary btn-bg">@lang('admin.add_syllable')</button>
                             </div>
                                <div class="table-responsive m-t-40">
                                    <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                        @foreach(\App\Syllabus::where('class_id',$id)->get() as $class)
                                            <tr>
                                                <td>{{$loop->iteration}}</td>
                                                <td>{{$class->session}}</td>
                                                <td>{{$class->term}}</td>
                                                <td>{{$class->subject}}</td>
                            <td><button onclick="showAjaxModal('{{url('')}}/admin/download/syllabus/{{$class->id}}')" class="btn btn-default btn-sm btn-block"><i class="fa fa-plus"></i>@lang('admin.print')</button></td>
                <td><button onclick="showAjaxModal('{{url('')}}/admin/show/syllabus/{{$class->id}}')" class="btn btn-info btn-sm btn-block"><i class="fa fa-book"></i>@lang('admin.edit')</button></td>
                    <td><button onclick="deletesyllable('{{$class->id}}')" class="btn btn-danger btn-sm btn-block"><i class="fa fa-book"></i>@lang('admin.delete')</button></td>
                                            </tr>
                        @endforeach
                                        </tbody>
                                    </table>
      ...
      <form action="{{url('')}}/add/new/syllabus/{{$id}}" method="post" enctype="multipart/form-data" class="">
         {{ csrf_field() }}
                  <div class="row">
                    <div class="col-sm-12">
                        <div class="card">
                            <div class="card-body">
<h4 class="card-title">@lang('admin.add_syllable') {{\App\Course::find($id)->title}}</h4>
                <h6 class="card-subtitle"></h6>
         <div class="row">
                        <div class="col-md-4">
...
<option value="{{\Carbon\Carbon::today()->subYear(0)->format('Y')}}/{{\Carbon\Carbon::today()->subYear(-1)->format('Y')}}">{{\Carbon\Carbon::today()->subYear(0)->format('Y')}}/{{\Carbon\Carbon::today()->subYear(-1)->format('Y')}}</option>
 ...
 document.getElementById('showtext').value =$(".mymce").val();
      ...
      function deletesyllable(id)
              {
                if(confirm("@lang('admin.confirm_delete')")){ 
                $.get('{{url('')}}/delete/syllable/'+id,function(data)
                {
                if(data==0){
                  swal("@lang('admin.account_delete')", "@lang('admin.continue-button')", "success");
                    window.location.assign('{{url('')}}/admin/page.fmp?page=55');
                }
            else{
