<div class="row">
                    <div class="col-sm-12">
                        ...
                        <div class="card card-body">
                            <h4 class="card-title">@lang('admin.certificate')</h4>
                            <h6 class="card-subtitle">{{\App\Graduation::find($id)->name}} C@lang('admin.certificate')</h6>
  <button onclick="add_medicine()" class="btn btn-primary btn-sm btn-block" ><i class="fa fa-plus"></i> @lang('admin.add') @lang('admin.certificate')
                                                    </button>
                                     <br>
<form class="" action="{{url('')}}/add/graduate/certificates/{{$id}}" enctype="multipart/form-data" method="post">
     {{ csrf_field() }}
                               <span id="medicine">
    @foreach(\App\Certificate::where('grad_id',$id)->get() as $ent)
                         <div class="form-group">
                        <div class="col-sm-12">
                          <div class="input-group">
                   <input name="title[]" value="{{$ent->title}}" placeholder="Result Title" type="text" class="form-control col-md-7 col-xs-12">
                  <input name="file[]" type="file" class="form-control col-md-7 col-xs-12">
                            <span class="input-group-btn">
                        <a href="{{url('')}}/download/cfile?file={{$ent->id}}" class="btn btn-curve btn-lg btn-info">@lang('admin.download')</a>
                                          </span>
                          </div>
                        </div>
                      </div>
    ...
    var medicine_count      = {{count(\App\Certificate::where('grad_id',$id)->get())}};
    var total_amount        = 0;
