...
@section('content')
    ...
    <div class="row page-titles">
                    <div class="col-md-5 col-8 align-self-center">
                        <h3 class="text-themecolor">@lang('admin.parent') @lang('admin.list')</h3>
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{url('')}}/parent/page.fmp?page=1">@lang('admin.home')</a></li>
                            <li class="breadcrumb-item active">@lang('admin.parent') @lang('admin.list')</li>
                        </ol>
                    </div>
 ...
 <div class="row">
                 ...
                 <div class="alert alert-danger alert-rounded">{{ session('error') }}</div>
                    @endif
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title">@lang('admin.parent') @lang('admin.export')</h4>
                                <h6 class="card-subtitle">@lang('admin.event-list-data')</h6>
                                 <div class="pull-right">
                                 <button data-toggle="modal" data-target=".bs-example-modal-lg" class="btn btn-primary btn-bg">@lang('admin.add') @lang('admin.parent')</button>
                             </div>
       ...
       <table id="example23" class="display nowrap table table-hover table-striped table-bordered " cellspacing="0" width="100%">
                      ...
                      <tbody>
                @foreach(\App\Parenting::where('status','0')->get() as $batch)
                        <tr>
                            <td>{{$loop->iteration}}</td>
        <td>{{$batch->name}}</td>
        <td>{{$batch->phone}}</td>
                        <td>{{$batch->address}}</td>
                      <td>{{count(\App\Relation::where('parent_id',$batch->id)->get())}}</td>
<td><button onclick="showAjaxModal('{{url('')}}/edit/parent/profile/{{$batch->id}}')" class="btn btn-sm btn-secondary btn-block"><i class="fa fa-book"></i> @lang('admin.edit')</button></td>
...
<div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="display: none;">
                    ...
                    <div class="col-sm-12">
                        <div class="card">
                            <div class="card-body">
<h4 class="card-title">@lang('admin.add') @lang('admin.new') @lang('admin.parent')</h4>
                <h6 class="card-subtitle"></h6>
    <form action="{{url('')}}/add/new/parent" method="post" enctype="multipart/form-data" class="">
         {{ csrf_field() }}
         <div class="row">
                        <div class="col-md-4">
        ...
        <div class="row">
                        ...
                        <div class="col-md-4">
                          ...
                          <select multiple class="form-control select2" style="width: 100%;" name="student[]">
                            <option value="">@lang('admin.select') @lang('admin.student')</option>
...
<script type="text/javascript">
              function deletesubject(id)
              {
                ...
                $.get('{{url('')}}/delete/parent/'+id,function(data)
                {
               if(data==0){
                  swal("@lang('admin.account_delete')", "@lang('admin.continue-button')", "success");
                    window.location.assign('{{url('')}}/admin/page.fmp?page=22');
                }
            else{
