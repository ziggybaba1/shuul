 ...
 @elseif(\App\Batch::find($id)->batch=='8')
    Batch H
    @endif) {{\App\Course::find(\App\Batch::find($id)->class)->title}} @lang('admin.batch_sperm')</h4>
                            <h6 class="card-subtitle">{{\App\Subject::find(\App\Batch::find($id)->subject)->title}}</h6>
 ...
 <div class="table-responsive m-t-40">
 	  <table id="exampler" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                     <input name="type" style="display: none;" type="number" required value="1"  class="form-control" >    	
                            @foreach(\App\Student::where('class',\App\Batch::find($id)->class)->get() as $staff)
