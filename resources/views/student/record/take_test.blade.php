...
@elseif(count(\App\Clockin::where('batch_id',$id)->where('student_id',\Auth::user()->id)->get())==0)
    <div class="col-12">
      ...
      <h3 class="card-title"><b>@lang('admin.subject'): {{\App\Subject::find(\App\Batch::find($id)->subject)->title}}</b></h3>
                                <h6 class="card-subtitle"></h6>
<form class="formsubmito" id="formsubmito" action="{{url('')}}/submit/test/answer/{{$id}}" method="post">
     {{ csrf_field() }}                              
...
<div class="row">
             ...
             <div class="col-sm-6"> 
                ...
                <input type="submit" required class="form-control btn btn-primary" value="@lang('admin.submit') @lang('admin.test')">
                    </div>  
                </div>
                 <div class="col-md-3"></div>
                        </div>
...
function clockin(){
   $.get('{{url('')}}/clockin/test/{{$id}}', function(data) {
            if(data=='0'){
