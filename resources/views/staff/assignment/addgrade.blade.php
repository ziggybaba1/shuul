...
<h3 class="text-themecolor">Add Assignment Grade for {{\App\Course::find(\App\Teacher::where('user_table',\Auth::user()->id)->first()->assign)->title}}</h3>
                        <ol class="breadcrumb">
                 ...
                 <div class="alert alert-info alert-rounded">{{ session('message') }}</div>
                    @endif
                        <div class="card card-body">
                            <h4 class="card-title">Assignment Grade</h4>
...
<option value="{{\Carbon\Carbon::today()->format('Y')}}/{{\Carbon\Carbon::today()->addYear(1)->format('Y')}}">{{\Carbon\Carbon::today()->format('Y')}}/{{\Carbon\Carbon::today()->addYear(1)->format('Y')}}</option>
  ...
  <label for="example-email">Enter Test Overall Score * (6)<span class="help"></span></label>
         ...
         <div class="col-sm-3"></div>
                    <div class="col-sm-6">
                 <div class="form-group">
                <input type="submit" required class="form-control btn btn-primary" value="Add Assignment Grade">
                    </div>  
                </div>
                        </div>
                        </form>
