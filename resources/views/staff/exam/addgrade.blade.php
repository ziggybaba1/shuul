...
<h3 class="text-themecolor">Add Exam Grade for {{\App\Course::find(\App\Teacher::where('user_table',\Auth::user()->id)->first()->assign)->title}}</h3>
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="javascript:void(0)">Home</a></li>
                            <li class="breadcrumb-item active">Exam Grade</li>
                        </ol>
                    </div>
                 ...
                 <div class="alert alert-info alert-rounded">{{ session('message') }}</div>
                    @endif
                        <div class="card card-body">
                            <h4 class="card-title">Exam Grade</h4>
                            <h6 class="card-subtitle">Add Exam Grade</h6>
<form class="" action="{{url('')}}/add/exam_grade/create" method="post">
    @csrf
    <div class="row">
       <div  class="col-sm-4">
...
<option value="{{\Carbon\Carbon::today()->format('Y')}}/{{\Carbon\Carbon::today()->addYear(1)->format('Y')}}">{{\Carbon\Carbon::today()->format('Y')}}/{{\Carbon\Carbon::today()->addYear(1)->format('Y')}}</option>
                  ...
                  <div class="col-sm-4">
                             <div class="form-group">
  <label for="example-email">Enter Student Exam Score * (5)<span class="help"></span></label>
           <input type="number" class="form-control" name="score">
                                </div>
                              </div>
                              <div class="col-sm-4">
                             <div class="form-group">
  <label for="example-email">Enter Exam Overall Score * (6)<span class="help"></span></label>
         ...
         <div class="col-sm-3"></div>
                    <div class="col-sm-6">
                 <div class="form-group">
                <input type="submit" required class="form-control btn btn-primary" value="Add Test Grade">
                    </div>  
                </div>
                        </div>
                        </form>
